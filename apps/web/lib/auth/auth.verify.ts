...
class MemoryAuthStore implements AuthStore {
  ...
  sessions: Array<
    AuthenticatedIdentity & { tokenHash: string; csrfTokenHash: string; revokedAt: Date | null }
  > = [];
  ...
  countRecentChallenges(walletAddress: string | null, since: Date) {
    return Promise.resolve(
      this.challenges.filter(
        (item) =>
          (walletAddress === null || item.address === walletAddress) &&
  ...
  findChallengeByAttemptHash(attemptHash: string) {
    return Promise.resolve(
      this.challenges.find((item) => item.attemptHash === attemptHash) ?? null
    );
  }
  completeAuthentication(
    ...
    const challenge = this.challenges.find(
      (item) =>
        ...
        item.attemptHash === input.attemptHash &&
        !item.consumedAt &&
  ...
  revokeSession(tokenHash: string, csrfTokenHash: string, at: Date) {
    const session = this.sessions.find(
      (item) =>
        item.tokenHash === tokenHash && item.csrfTokenHash === csrfTokenHash && !item.revokedAt
    );
...
check(
  "nonce creation is persisted and audited",
  store.challenges.length === 1 && store.audits[0]?.eventType === "SIWE_NONCE_CREATED"
);
check(
  "nonce has at least 128 bits of entropy",
  /^[a-f0-9]{32}$/.test(new SiweMessage(challenge.message).nonce)
);
...
check(
  "SIWE message is canonical",
  parsed.domain === config.domain &&
    parsed.uri === config.uri &&
    parsed.version === "1" &&
    parsed.chainId === 97 &&
    parsed.statement === "Sign in to BNB Agent Marketplace."
);
...
check(
  "wallet ownership created",
  success.identity.walletAddress === address && success.identity.chainId === 97
);
check(
  "session token is hashed before storage",
  store.sessions[0]?.tokenHash === sha256(success.sessionToken) &&
    store.sessions[0]?.tokenHash !== success.sessionToken
);
check(
  "raw token never appears in audit records",
  !JSON.stringify(store.audits).includes(success.sessionToken)
);
const replay = await verifyAuthentication({
  store,
  attemptToken: challenge.attemptToken,
  message: challenge.message,
  signature,
...
const wrongNonce = new SiweMessage({
  ...parsed,
...
const expiredResult = await verifyAuthentication({
  store,
...
check(
  ...
  !expiredResult.ok && expiredResult.reason === "expired-challenge"
);
const invalidSignature = await verifyAuthentication({
  store,
  attemptToken: expired.attemptToken,
  message: expired.message,
  signature: "0x1234",
...
const signerMismatchSignature = await otherAccount.signMessage({
  message: signerMismatchChallenge.message,
});
const signerMismatch = await verifyAuthentication({
  store,
  attemptToken: signerMismatchChallenge.attemptToken,
  message: signerMismatchChallenge.message,
...
check(
  "signer/address mismatch rejection",
  !signerMismatch.ok && signerMismatch.reason === "wrong-wallet"
);

const authUser = await getAuthenticatedUserFromStore(
  store,
...
const expiredUser = await getAuthenticatedUserFromStore(
  store,
  success.sessionToken,
  new Date(success.expiresAt.getTime() + 1)
);
...
check(
  ...
  (await getAuthenticatedUserFromStore(
    store,
    success.sessionToken,
...
check(
  ...
  await store.revokeSession(
    sha256(success.sessionToken),
    sha256(success.csrfToken),
...
const conflict = await verifyAuthentication({
  store: conflictStore,
  attemptToken: conflictChallenge.attemptToken,
