...
// default and the scope-blind category discovery.
{
  ...
  // ---- X.245/X.246 — chain-56 hire availability (stale UI guard fix) ----
  {
    ...
    const cardSource = readFileSync(new URL("./card.ts", import.meta.url), "utf8");
    const hireApiSource = readFileSync(
      new URL("../activation/main-track-hire.api.ts", import.meta.url),
      "utf8"
    );
    // X.246D1: Agent 334760 (chain 56, owned) is recognized as hireable —
    // through the single predicate, at EVERY display site.
    check(
      "X.246D1 chain-56 registered agents get the REAL hire view at every display site (single isHireChain predicate)",
      /isHireChain\(agent\.chainId\) && agent\.ownerAddress/.test(detailSource) &&
        /isHireChain\(agent\.chainId\) && Boolean\(agent\.ownerAddress\)/.test(hireViewSource) &&
        /isHireChain\(agent\.chainId\) && Boolean\(agent\.ownerAddress\)/.test(cardSource)
    );
    check(
      "X.246D2 the stale 'Mainnet hiring coming soon' guard cannot return (absent everywhere)",
      !/Mainnet hiring coming soon/.test(detailSource + hireViewSource + cardSource)
    );
    check(
      "X.246D3 non-hireable registry agents remain truthful (honest owner-less fallback + card reason)",
      /Hire unavailable|no registered owner/.test(detailSource) &&
        /hireUnavailableReason/.test(cardSource)
    );
    check(
      "X.246D8 hire eligibility remains server-authoritative (X.241 flag + chain-aware API gate untouched)",
      hireApiSource.includes("mainnet-hire-disabled") &&
        /isMainnetHireEnabled\(input\.env\)/.test(hireApiSource)
    );
    check(
      "X.246 the predicate lives in the authoritative seam (hire-chains.ts), not duplicated per file",
      readFileSync(
        new URL("../../../../packages/integrations/src/altana/hire-chains.ts", import.meta.url),
        "utf8"
      ).includes("export function isHireChain") &&
        !/\(agent\.chainId === 97 \|\| agent\.chainId === 56\)/.test(
          detailSource + hireViewSource + cardSource
        )
    );
  }
