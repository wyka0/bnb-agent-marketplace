...
import { isHireChain } from "@bnb-marketplace/integrations/altana";
...
export function toAgentCardData(agent: LeaderboardAgent): AgentCardData {
  ...
  // X.160/X.246: Main Track Hire (Model B) is available for registered agents
  // on the two REAL commercial hire chains (97 Testnet + 56 Mainnet, both
  // hire-enabled in production since X.242) with a registered owner — via the
  // single authoritative `isHireChain` predicate (X.246: no more per-file
  // duplicated chain pins; X.245 missed this file, showing every Mainnet card
  // as "Unavailable"). The detail page negotiates with the live seller and
  // the server-side X.241 gate fails closed honestly; Model A ACTIVATABLE
  // remains capability-derived and unchanged.
  const mainTrackHireable = isHireChain(agent.chainId) && Boolean(agent.ownerAddress);
  ...
  return {
    ...
    hireUnavailableReason: hireable
      ? undefined
      : isHireChain(agent.chainId)
        ? "Main Track Hire requires a hire-chain (56 or 97) agent with a registered owner."
        : activation.detail,
  } as AgentCardData;
}
