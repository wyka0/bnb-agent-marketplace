...
export function LeaderboardsView({
  ...
  /** X.232 — discovery-network scope (read-only prop; switching is URL-driven). */
  scope?: "all" | "mainnet" | "testnet";
  /** X.232 — current catalog page (read-only prop; navigation is URL-driven). */
  page?: number;
}) {
  ...
  // X.232 — `network` state replaced by the URL-driven `scope` prop (server
  ...
  const resetAll = React.useCallback(() => {
    ...
    // X.232 — network scope is URL-driven; reset also clears it.
        ...
        <FilterGroup label="Network" role="radiogroup">
          {NETWORK_OPTIONS.map((n) => (
            <FilterRadio
              ...
              onSelect={(next) => {
                // X.232 — URL-driven scope switching (server re-reads); resets
    ...
    <MarketplaceContainer className="py-5">
      ...
      >
        <MarketplaceContent>
          ...
          {/* X.232 — truthful, bounded pagination from REAL upstream metadata.
          ...
          <div className="mt-6 flex flex-col items-center gap-2">
            ...
            {leaderboardHasMoreBeyondWindow ? (
              <p className="text-xs text-muted-foreground">
                Showing the 10 highest-scored pages —{" "}
