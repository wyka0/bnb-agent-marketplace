...
export function MainTrackHireView({ agent }: { agent: LeaderboardAgent }) {
  ...
  // X.245/X.246 — chain-aware availability via the single authoritative
  // `isHireChain` predicate (56 Mainnet + 97 Testnet, both hire-enabled in
  // production since X.242). The server-side X.241 gate remains
  // authoritative: a chain-56 prepare fails closed with the truthful
  // "Mainnet hiring is unavailable" response whenever the flag is off, so
  // this UI state can never bypass the backend.
  const isAvailableAgent = isHireChain(agent.chainId) && Boolean(agent.ownerAddress);
