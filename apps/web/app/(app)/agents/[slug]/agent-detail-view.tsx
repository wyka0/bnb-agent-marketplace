...
export function AgentDetailView({
  ...
  const hireCard =
    agent && isHireChain(agent.chainId) && agent.ownerAddress ? (
      /* X.245/X.246 — registered agents on the REAL hire chains (56 + 97,
         both hire-enabled in production since X.242) get the REAL hire flow
         via the single authoritative predicate. Mainnet hiring was enabled in
         production (X.242-DEPLOY-ENABLE: MAINNET_HIRE_ENABLED=true, seller
         live, first hire funded as Job 56715), so the X.234 "coming soon"
         card was a stale UI guard, not a safety gate. Actual availability
         remains governed server-side by the X.241 chain-aware API (a
         chain-56 prepare fails closed with the truthful "unavailable"
         response whenever the flag is false) — this card can still never
         bypass the backend. */
