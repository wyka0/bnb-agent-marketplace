...
export default function PermissionsPage() {
  ...
  const spendPermission = session?.permissions.find(
    (permission) => permission.kind === "TOKEN_SPEND"
  );
  const nativePermission = session?.permissions.find(
    (permission) => permission.kind === "NATIVE_SPEND"
  );
  const meta = session ? statusMeta(session, load) : null;
  const revokeDisabled =
    busy || session === null || session.status === "revoked" || session.status === "failed";
  ...
  return (
    <div className="container py-8">
      ...
      <div className="mb-8">
        ...
        <p className="mt-1 text-muted-foreground">Wallet permissions and session capabilities.</p>
      </div>

      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Wallet identity</CardTitle>
          <CardDescription>Connected via SIWE · BNB Testnet (chain 97)</CardDescription>
        </CardHeader>
        <CardContent>
          {identity ? (
            <div className="flex flex-wrap items-center gap-3">
              <span className="inline-flex items-center gap-1.5 rounded-md border border-primary/40 bg-primary/10 px-2.5 py-1.5 text-sm font-semibold text-primary">
                <span className="h-1.5 w-1.5 rounded-full bg-primary" aria-hidden="true" />
                <span className="font-mono">{shortenAddress(identity.walletAddress)}</span>
              </span>
      ...
      {session ? (
          ...
          <CardContent className="space-y-4 text-sm">
            <dl className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <dd>
                  {session.keyStoreActive
                    ? "Active"
                    : load?.reason === "key-store-unavailable"
                      ? "Unknown"
                      : "Inactive"}
                </dd>
              </div>
              ...
              <div>
                ...
                <dd>
                  {session.permissionLimitRaw} raw $U unit/day (
                  {spendPermission?.spendPeriod ?? "day"})
                </dd>
              </div>
              <div>
                <dt className="text-muted-foreground">Usage</dt>
                <dd>
                  {session.spentRaw} / {session.permissionLimitRaw} raw units (
                  {session.remainingRaw} remaining)
                </dd>
              </div>
              <div>
                ...
                <div>
                  ...
                  <dd>
                    ...
                    <a
                      ...
                      href={`https://testnet.bscscan.com/tx/${revokeTxHash}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      open in explorer
            ...
            <div className="flex flex-wrap items-center gap-3 border-t pt-4">
              ...
              <p className="text-muted-foreground">
                Revoking disables the current Altana session on BNB Testnet. Agent 1816 and Job 515
                are not affected.
              </p>
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Altana sessions</CardTitle>
            <CardDescription>Not configured on this deployment.</CardDescription>
          </CardHeader>
        </Card>
      )}
    </div>
