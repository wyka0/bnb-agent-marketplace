...
/**
 * X.246 — the single predicate for "is this one of the two REAL commercial
 * hire chains" (56 BNB Mainnet + 97 BSC Testnet — both hire-enabled in
 * production since X.242). Display code MUST use this instead of
 * hand-duplicating `chainId === 97 || chainId === 56` per file: X.245 fixed
 * two of three duplicated sites and missed card.ts, which made every
 * Mainnet catalog card show "Unavailable". This predicate is DISPLAY-ONLY;
 * actual hire eligibility always remains server-authoritative (the X.241
 * chain-aware API gate + MAINNET_HIRE_ENABLED flag).
 */
export function isHireChain(chainId: number): boolean {
  return chainId === HIRE_CHAIN_MAINNET || chainId === HIRE_CHAIN_TESTNET;
}
